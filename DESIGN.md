...
| Tab 条 | 高 36；Tab：图标 16 + 标题 `ui-md`，激活态底部 2px `primary` 指示线 + `surfaceContainerLow` 底；流式中 Tab 标题前置 8px running 状态点；溢出两段式（design-tab-overflow，2026-09-22 三修终案）：先等比例挤压（`flex-shrink:1`，min-width 96 兜底）→ 全体到最小宽仍放不下时尾部 Tab 入 overflow 菜单（容量 = (条宽−72)/96 纯公式，激活/拖拽项保位恒可见），「+」（lucide `plus`）/ overflow（lucide `chevrons-right`）常驻钮 `flex-shrink:0` 永不挤出；前两轮滚动方案（可见滚动条占位 8px 挤压 Tab 高度、隐藏滚动条可发现性差）实测推翻，迭代史见 design-tab-memory §11 |
...
| 文字选区（全局，2026-09-18） | 原生 `::selection` 全局令牌化：派生 token `--selection-tint`（tokens.css 唯一落点）= `primary` 26% 透明淡染（`color-mix`），替换 UA 默认系统蓝（与绿调设计冲突，深浅主题均不适配）；`primary` 随主题翻转自适应——深色（#98d4a3）混得选中底 L*≈30 对正文 ~5.9:1、浅色（#35693e）混得 L*≈81 对正文 ~7.8:1，量级同 GitHub 原生选区，正文原色不翻转；`::selection` 与 `::highlight(md-find)`（markdown 预览查找高亮，原同值独立副本）同源消费该 token，code-view 原局部选区规则同日收编；终端（恒深色）`selectionBackground` 为 JS 常量无法消费 CSS token，硬编码对齐同值 `#98d4a342`（原 outline-variant `#41494188`） |
...
实现基类 `app.css` 的 `.dialog` / `.dialog-mask`（2026-08-31 起为本标准），全部弹窗复用同一骨架，变体只落尺寸类；五类现役弹窗：设置（`dialog-lg`）、打开项目（`dialog-project`）、打开方式（`open-with-dialog`，2026-08-31 对齐）、确认（`dialog-sm`）、独立连接（`connecting-dialog`，2026-09-23——新建/切换服务器挂起期独占屏幕）。设置弹窗内的服务器添加/编辑为弹窗内视图跳转（见下方「模态不重叠」）。
...
| 连接 | `.connecting-dialog` | 320，随内容高（spinner + 文案水平排布居中，无标题/动作钮） | 独立连接弹窗（2026-09-23） |
...
**键盘与遮罩**（交互惯例，全弹窗一致）：遮罩点击 = 关闭（在途操作型除外，如确认弹窗 loading 中禁点；独立连接弹窗全程无操作——连接不可中断，见 design-guided-add-server 修订 3）；Esc 关闭；搜索框打开即聚焦、`↑↓/Enter` 操作列表、输入框内 IME 组合（`isComposing`）的 Enter/Escape 不触发选择/关闭；含过滤搜索的弹窗 Esc 语义 = 有内容先清空、再按关闭。
