...
- **显示环境注入（2026-09-11 增）**：createPty body 附 `env` = 主进程显示会话**白名单切片**（`pty:displayEnv` IPC → main `pickPtyDisplayEnv`：DISPLAY/WAYLAND_DISPLAY/XAUTHORITY/XDG_SESSION_TYPE/XDG_RUNTIME_DIR/XDG_CURRENT_DESKTOP/DBUS_SESSION_BUS_ADDRESS）。依据：server 侧 pty env = `...process.env`（server 进程环境，core pty.ts）完全继承，server 若自非图形上下文启动（实证：本机 opencode-serve **systemd user 服务**先于 GNOME 的 DISPLAY/WAYLAND_DISPLAY import 启动，快照永久缺显示变量），终端内 GUI 程序（`smerge .` 等）连不上显示静默失败；Electron 主进程必在图形会话内，server 合并序 **payload env 优先于 process.env**（handlers/pty.ts `{...ctx.payload.env, ...shell.env}` → core `{...process.env, ...input.env}`）故注入即回填。**仅回环 server 注入**（`isLoopbackBaseUrl`，managed 随机端口天然回环）：远程 attach 的 DISPLAY 语义属远端（ssh -X 转发的 `localhost:10.0` 会被本地值顶掉）。**已知盲区（review 2026-09-11 登记）**：`ssh -L` 端口转发的 baseUrl 也是回环——本地显示变量会注入到隧道对端的 pty（覆盖远端自身显示配置），启发式无法区分；频率低且失败形态与未注入场景一致（GUI 开不了窗），接受为 heuristic，如真机报障再议按 profile 显式开关。白名单而非全量透传：body.env 会覆盖 server 语义，只补显示必需键；空切片（无显示环境）/IPC 失败 = 不注入（终端创建不受阻）；纯浏览器 shim 恒空。运维侧正解（与本注入互补）：unit 加 `After=graphical-session.target` 或图形会话内 `systemctl --user restart`
...
- **live 态 Tab 切换释放（2026-09-10 修订，推翻原"Ctrl 系全归 pty"无例外语）**：非 mac 的 `Ctrl+Tab` / `Ctrl+Shift+Tab` 在 live 态也 `return false` 释放给应用切 Tab。依据：xterm 对 Tab 类按键**忽略 ctrl 修饰**——Ctrl+Tab 发 `\t`、Ctrl+Shift+Tab 发 `CSI Z`，与裸 Tab / Shift+Tab 发出的字节完全相同（CLI 无法感知 ctrl 是否按下），且无主流 CLI 绑定 Ctrl+Tab（shell 补全用裸 Tab；zsh 补全菜单反向遍历 `bindkey '^[[Z'` 的实际键位是 Shift+Tab，仍透传不受影响）——释放零损失。边界：仅非 mac（mac 无 Ctrl+Tab 系绑定，切 Tab 走 ⌘⌥←/→ 惯例键，shortcuts §1——释放只丢 Tab 键入无收益；mac 终端内 ⌘ 系组合仍归 pty、切 Tab 不可用的已有限制维持）；排除 alt/meta（Ctrl+Alt+Tab 非 dispatch 的 Tab 分支——`!alt` 守卫，不放行才不留"释放了却切不了 Tab"的空洞）。**Ctrl+W 维持归 pty（用户决策 2026-09-10）**：readline `backward-kill-word`（0x17，bash/zsh 高频删词）、vim 窗口命令前缀（C-w s/v/w/o/h…）、nano 搜索、emacs kill-region 都依赖它；且关终端 Tab 即销毁 pty（Ctrl+Shift+T 只能原 cwd 新建，进程现场全丢），误按代价过高——业界同例：GNOME Terminal/Konsole 不绑 Ctrl+W，Windows Terminal 关标签用 Ctrl+Shift+W，VS Code 集成终端聚焦时 Ctrl+W 留给 shell。Ctrl+PgUp/PgDn（同族切 Tab 键）一并维持归 pty
- **断开态不拦截应用快捷键**（2026-09-04 修订）：live 态终端聚焦时 Ctrl/⌘ 系组合归 pty（xterm 对这类键 preventDefault+stopPropagation 且监听挂 textarea capture——全局分发 shortcuts.ts 收不到事件，Ctrl+W 等被吞，是既定语义；**Tab 切换系例外——live 态也释放，见上条 2026-09-10 修订**）；但**无 OPEN 的 WS 时**（连接中/重连中/已退出/已断开）按键无处可去（onData 只发 OPEN 态），拦截只剩副作用——customKeyEventHandler 对带 Ctrl/⌘ 的组合与裸 Alt+↑/↓（非 mac 作用域遍历，shortcuts §3）`return false` 释放给应用快捷键（Ctrl+W 关 Tab、Ctrl+Tab / **Ctrl+Shift+Tab** 切 Tab、Ctrl+B 面板开关等生效——含 Shift 的组合也释放；关 Tab 免确认判定 §1.1 早备）。唯二例外是复制/粘贴组合（C/V）——断开态仍走拦截（可复制回滚选区）。无修饰键仍归 xterm（键盘滚动等默认行为保留）。**Alt 遍历预览-提交（2026-09-06，shortcuts §3 修订）**：裸 Alt keydown/keyup 本就冒泡（xterm 不消费纯修饰键，live/断开皆然）——live 态 Alt+↑/↓ 归 pty，光标显示但不移动、松开 no-op，属预期；断开态 Alt+↑/↓ 经 deadRelease 释放，预览-提交完整可用。
