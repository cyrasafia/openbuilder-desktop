...
- **无激活 Tab 的 Ctrl+W 也消费**（2026-08-29 修订，推翻原"无加速键冲突"断言）：Electron 默认菜单并未因 autoHideMenuBar 消失，其 role:close 的 Ctrl+W 加速键对 **renderer 未消费**的按键生效（Chromium 对 renderer 未处理的键回调 `HandleKeyboardEvent` 触发加速键）——实测无 Tab 时 Ctrl+W 直接把窗口关掉。故 dispatch 对无激活 Tab 的 Ctrl+W 返回"已消费"（preventDefault、不动作）；其余未映射组合仍放行。Ctrl+数字跳转不做（用户决策，系统/输入法易冲突）。**浏览器/PDF 视图持焦时同一漏洞（2026-09-19 修复）**：renderer 的 window keydown 不可达、preventDefault 失效——view 的 before-input-event 转发只驱动关 Tab 动作不消费按键，未消费键回流命中 Window>Close 加速键，Ctrl+W 伴随整窗关闭（本地 file:// 页面实测复现）。修复在 view 的 before-input-event 吞 Ctrl/⌘+W（design-browser-tab §1.2，判定抽 `src/main/tab-close-shortcut.ts`），与"无激活 Tab 也吞"同为"禁用而非放行"语义；**同日 review 修订：dispatch 的 W 分支改按 code `KeyW` 匹配 + key 字面双保险（KeyB 先例）**——原仅按 key 匹配，非拉丁布局（俄语等物理 KeyW 上报本地字符）时主窗口路径同样不消费、放行回流同一加速键关窗
...
- **动作与磁贴一致**：Ctrl+1 → `openDiffTab()`、Ctrl+2 → `openTerminalTab()`、Ctrl+3 → `openNewBrowserTab()`（每次新开空白网页 Tab，不去重——2026-09-15 修订，原 `openBrowserTab("about:blank")` 复用首 Tab 见 design-browser-tab §1.3），与磁贴点击同路径、**同禁用态**（无 activeProfile 时 2/3 不动作；browser shim 平台 3 不动作）
