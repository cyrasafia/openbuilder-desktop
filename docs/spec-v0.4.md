...
- [ ] 模型页签（#4 增补，design-model-list）：已配置供应商的模型按 provider 分组列出（搜索过滤可用）；行开关即时生效——关闭后 composer/默认值工具条模型 picker 不再列出该模型（分组计数收缩），已是当前会话模型的 pill/thinking 照常；生效默认（新会话/引导页展示）被关后回退首个开启模型；重开应用开关状态保留（per-profile）；删除服务器后其切片清除、其余服务器不受扰
