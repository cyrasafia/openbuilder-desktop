...
| 4 | Provider/Model 配置 | 设置弹窗新增 **Provider 页签**：**已配置 provider 列表**（`key` 非空/已连接/自定义 source 者才展示——2026-09-23 修订：全目录搜索与手动刷新移除，见 [design-provider-config.md](./design-provider-config.md) §2；名称、source、key 配置状态、模型数，列表数据源 `GET /provider` 按当前作用域目录查，spec 原映射 `GET /config/providers` 实测不含未配置项已弃用）+ **API key 设置/删除**（`PUT /auth/{providerID}` `{type:"api", key}` / `DELETE /auth/{providerID}`，仅 API key 形态）。**Model 配置 = 默认模型选择**（复用现有「默认」页签 agent/model）。**模型列表**（**2026-09-23 增补**，见 [design-model-list.md](./design-model-list.md)）：设置弹窗新增「模型」页签——按 provider 分组列出已配置供应商全部模型（数据源 = picker 同一 `/config/providers` 目录缓存，无新增 API），每模型行开关：关 = 从模型选择列表（composer/默认值工具条）消失不可选，开 = 恢复；组头收起/展开 + 组级全部开启/全部关闭（语境单钮，批量单次落盘）；已是当前会话模型的照常工作；生效默认解析在过滤后列表上进行（被关默认回退首个开启模型）。开关状态**本地保存、跟服务器（profile）走**：`models.disabled` 持久化（per-profile 例外集，缺省全开），**删除服务器（profile）时清理切片**；模型级陈旧条目惰性无效不修剪。范围外见下（OAuth、config 编辑等） |
...
| 10 | 页面内搜索（**2026-09-09 增补**） | markdown 预览态 / 浏览器 Tab / PDF 文件 Tab 增加页面内搜索（[design-find-in-page.md](./design-find-in-page.md)）：Ctrl+F（mac ⌘F）唤起三视图共用的自建查找条（输入即搜 + `n/m` 计数 + Enter/Shift+Enter 前后跳 + Esc 关闭清高亮；无匹配输入框描红）。markdown 预览态 = renderer DOM 文本扫描（TreeWalker）+ CSS Custom Highlighting 高亮（零 DOM 变更，不侵入 streamdown 托管树）+ 环绕跳转滚动定位；浏览器/PDF = 原生 `webContents.findInPage`（Chromium 选区高亮零 DOM 成本，新增 browser:find-start/stop IPC + found-in-page 末帧推送 + requestId 旧帧守卫，导航自动清零）。唤起通道 = 全局分发 Ctrl+F 分支 + store 注册表（Tab 注册制：视图挂载注册/卸载注销，代码视图 CM 自持搜索不注册故放行；浏览器/PDF 视图持焦时经既有 browser:shortcut 转发同分发）。状态为瞬时任务态：不持久化、切走重挂载重搜 |
| 11 | 输入草稿跨重启持久化（**2026-09-11 增补**） | 会话 Tab 与引导页输入框的未发送文本草稿，关闭应用再打开仍保留（[design-compose-draft.md](./design-compose-draft.md) §5 磁盘层，修订原 v0.1「不持久化」决策）：`store.json` 新增 `drafts.state`（profileKey 分切片）；connect 时播种内存 map（组件零改动）、键入 500ms 去抖落盘 + 快照去重、teardown 清空前按连接切片键冲刷（断连/切 profile 不丢且不串）、pagehide 即时冲刷 + main `will-quit` 同步写兜底退出截断；清理语义磁盘与内存同灭（发送成功/关 Tab/目录卸载后重启不复活）。仅文本草稿——引用/附件不持久化 |
| 12 | 窗口变窄自动收放左右栏（**2026-09-16 增补**） | 窗口宽→窄跌破阈值（当前展开栏合计 + 中栏下限 648，与 app.css `--chat-min` 同源动态计算）时左右栏**同时**自动收起，回宽过阈值+64 回差自动放回（[design-layout-collapse.md](./design-layout-collapse.md) §2.6，spec-v0.3 #1 折叠能力的自动化补全）：收起按宽→窄**边沿**触发（收起后窄区间内 resize 不重评，窄窗手动展开不被抢收）；手动收起的栏回宽不放回、手动 toggle 即接管本栏；自动收起态不持久化（`persistLayout` 落盘过滤 auto 标记，重启按用户意图还原、窄窗初评再收）；面板拖拽调宽不触发 |
...
- [ ] 全新数据目录启动（无 profile）出现全页欢迎屏（无三栏）：入口页仅「添加服务器」（无设置入口，2026-09-08 修订；未进入流程不扫描）；点击进入发现视图双扫描混排（运行中 server + 本机 opencode 候选，多行卡片按模式标题区分 + 明细行地址/路径 + 版本，2026-09-06 修订），点 managed 候选一键启动连接、attach 候选一键连接；连接成功直接进主界面（无 provider/默认模型引导），中栏为「打开项目」引导页；删除全部 profile（重启或运行中）欢迎屏复现；已有激活 profile 启动不出现
...
- [ ] 模型页签（#4 增补，design-model-list）：已配置供应商的模型按 provider 分组列出（无搜索框/手动刷新/提示行，2026-09-23 精简——失败态提示可点击重试）；组头收起/展开（瞬时态不重启保留）与组级全部开启/关闭（语境单钮，批量单次落盘），列表滚动时组头条吸顶（下一组头无缝顶替）；行开关即时生效——关闭后 composer/默认值工具条模型 picker 不再列出该模型（分组计数收缩），已是当前会话模型的 pill/thinking 照常；生效默认（新会话/引导页展示）被关后回退首个开启模型；重开应用开关状态保留（per-profile）；删除服务器后其切片清除、其余服务器不受扰
...
- [ ] 页面内搜索（#10）：markdown 预览态 Ctrl+F 唤起查找条（视图底部，与代码视图 CM 搜索面板同位同 idiom——2026-09-09 居底统一样式）、输入即高亮全部匹配（当前匹配深色）+ 计数 + Enter/Shift+Enter 环绕跳转滚动定位 + Esc 关闭清高亮；源码态/代码文件 Ctrl+F 无需先聚焦正文即唤起 CM 搜索面板（2026-09-09 修订，注册回调 openSearchPanel）、图片/二进制 Ctrl+F 放行；浏览器 Tab 页面内 Ctrl+F（经转发）与应用侧同分发唤起、原生高亮计数、导航后计数清零；PDF Tab 唤起/高亮/计数正常（扫描件 0 匹配输入框描红）；浏览器 shim Ctrl+F 无动作
