...
| 4 | Provider/Model 配置 | 设置弹窗新增 **Provider 页签**：provider 列表（名称、source、key 配置状态、模型数，`GET /config/providers` 按当前作用域目录查）+ **API key 设置/删除**（`PUT /auth/{providerID}` `{type:"api", key}` / `DELETE /auth/{providerID}`，仅 API key 形态）。**Model 配置 = 默认模型选择**（复用现有「默认」页签 agent/model）。**模型列表**（**2026-09-23 增补**，见 [design-model-list.md](./design-model-list.md)）：设置弹窗新增「模型」页签——按 provider 分组列出已配置供应商全部模型（数据源 = picker 同一 `/config/providers` 目录缓存，无新增 API），每模型行开关：关 = 从模型选择列表（composer/默认值工具条）消失不可选，开 = 恢复；组头收起/展开 + 组级全部开启/全部关闭（语境单钮，批量单次落盘）；已是当前会话模型的照常工作；生效默认解析在过滤后列表上进行（被关默认回退首个开启模型）。开关状态**本地保存、跟服务器（profile）走**：`models.disabled` 持久化（per-profile 例外集，缺省全开），**删除服务器（profile）时清理切片**；模型级陈旧条目惰性无效不修剪。范围外见下（OAuth、config 编辑等） |
