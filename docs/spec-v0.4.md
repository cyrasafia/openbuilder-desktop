...
| 4 | Provider/Model 配置 | 设置弹窗新增 **Provider 页签**：**已配置 provider 列表**（`key` 非空/已连接/自定义 source 者才展示——2026-09-23 修订：全目录搜索与手动刷新移除，见 [design-provider-config.md](./design-provider-config.md) §2；名称、source、key 配置状态、模型数，列表数据源 `GET /provider` 按当前作用域目录查，spec 原映射 `GET /config/providers` 实测不含未配置项已弃用）+ **API key 设置/删除**（`PUT /auth/{providerID}` `{type:"api", key}` / `DELETE /auth/{providerID}`，仅 API key 形态）。**Model 配置 = 默认模型选择**（复用现有「默认」页签 agent/model）。范围外见下（OAuth、config 编辑等） |
