...
- [ ] 页面内搜索（#10）：markdown 预览态 Ctrl+F 唤起查找条（工具条下方）、输入即高亮全部匹配（当前匹配深色）+ 计数 + Enter/Shift+Enter 环绕跳转滚动定位 + Esc 关闭清高亮；源码态 Ctrl+F 走 CM 既有面板不受影响、代码/图片/二进制 Ctrl+F 放行；浏览器 Tab 页面内 Ctrl+F（经转发）与应用侧同分发唤起、原生高亮计数、导航后计数清零；PDF Tab 唤起/高亮/计数正常（扫描件 0 匹配输入框描红）；浏览器 shim Ctrl+F 无动作
