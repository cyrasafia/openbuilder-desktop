...
- 源码态切换、缩放/页码控制（PDFium 自带工具条）、打印
- ~~搜索~~ 已做（2026-09-09）：应用内查找条走 `webContents.findInPage`（design-find-in-page）——「缩放/页码不做、查找在应用内做」的理由：查找有跨视图统一的 Ctrl+F 习惯与共用查找条诉求（markdown/浏览器同语义），缩放/页码是 PDFium 工具条的舒适区无此诉求
