...
- **折叠体系（2026-08-27 修订）**：原设计含 hunk 级 + 文件级两级折叠，**已移除 hunk 级折叠**（实际使用中文件级折叠已足够，hunk 折叠增加交互复杂度且与文件折叠语义重叠），仅保留文件级折叠——
  ...
  - **文件级 + 全局（2026-09-11 修订：默认意图 + 覆盖表单一事实源）**：文件块头部（`<button>` + chevron，`aria-expanded`）点击收起/展开本文件；工具条右侧「全部折叠 / 全部展开」（`btn-tonal` chip，仅渲染于有文件且非错误态；右对齐，与 segment 同高）。开合模型 = DiffView 级 `foldOpen: boolean`（默认意图）+ `fileOpens: Map<path, boolean>`（手动覆盖表），`open = fileOpens.get(path) ?? foldOpen` 全派生（FileDiffBlock 无本地开合 state）——手动开合写覆盖表（新值等于默认意图时删条目）；全部折叠/展开翻转意图**并清空覆盖表**（新意图覆盖一切手动增量）。意图只表达按钮交替方向、不追踪覆盖表（手动折叠不改变按钮标签）；数据刷新（激活即重拉）不重置手动状态，新出现的文件块无覆盖条目、继承当前意图。
  - **视图状态持久化（2026-08-27 增补，2026-09-11 修订）**：切 Tab 卸载前落 store（`diffViewStates`，design-tab-state-memory §2.5），重挂载恢复——`foldOpen`（全局折叠意图）、`fileOpens`（逐文件开合覆盖表）、`scrollTop`（滚动偏移）。卸载经 ref 读最新值 + 复活闸门（Tab 仍在才写）；恢复 foldOpen/fileOpens 经 `useState` 初始化值、scrollTop 经 `useLayoutEffect` 在内容落地后应用。~~首次挂载跳过 foldOpen 覆盖（`firstMount` ref）~~（修订后随本地开合 state 一并移除——开合全派生，无覆盖时序问题）。
