...
- [ ] 终端断线（server 侧 pty 仍活）自动重连续传不丢不重，重连中 banner 出现/消失；窗口 focus 立即重试；pty exit 或 server 重启（token 404/close 4404）呈已退出终态
