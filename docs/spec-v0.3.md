...
| # | 功能 | 说明 |
|---|------|------|
| 1 | 栏收起/展开 | 左/右栏收起后**完全不展示**（无残留窄条）；收起/展开按钮放在标题栏（导航条）右侧、最小化按钮左边，左右栏各一个。**快捷键 Ctrl+[ / Ctrl+] 分别收起/展开左/右栏**（翻转语义；终端 Tab 聚焦时键归 xterm 不生效）。折叠态与各栏宽度持久化（`layout.state` IPC 键），重启还原；补齐拖拽调宽 |
...
- [ ] 左/右栏可收起展开（收起后完全不展示），宽度与折叠态重启还原；拖拽调宽生效；Ctrl+[ / Ctrl+] 分别翻转左/右栏（终端聚焦时除外）
