...
- [ ] 点击 .html 开浏览器 Tab 渲染正确（本地相对资源可加载）；右键「查看源码」在文件 Tab 打开源码；地址栏导航、前进/后退、打开本地文件可用；远端页面无法跳转 `file://`；新开空白 Tab 显示欢迎页（地址栏空、标签条标题「新标签页」、原生视图隐藏），地址栏/欢迎页入口导航后欢迎页消失、视图显示（2026-09-18 增）
