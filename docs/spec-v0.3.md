...
| 6 | 浏览器 Tab | WebContentsView 内嵌浏览器（地址栏 + 前进/后退/刷新 + 打开本地文件），Tab 归作用域；**地址栏无 scheme 输入自动补全（2026-09-19 增）：裸点分域名（含 IDN）补 `https://`，`localhost`/IPv4/IPv6 字面量补 `http://`，其余维持字面 file 路径**。**新开空白 Tab（磁贴/Ctrl+3/恢复空白/关闭栈重开空白）不加载 about:blank，内容区显示欢迎页（2026-09-18 增：图标 + 提示 + 打开本地文件入口，地址栏空、标签条标题本地化「新标签页」），导航后恢复常规浏览器视图**；**文件树点击 `.html/.htm` 默认在浏览器 Tab 打开（`file://` URL）**；右键菜单提供「查看源码」，在文件 Tab 打开 HTML 源码（文件 Tab 仅源码、无预览，原 iframe 预览方案废弃）。导航安全：远端页面禁跳 `file://`，外链走系统浏览器 |
...
- [ ] 点击 .html 开浏览器 Tab 渲染正确（本地相对资源可加载）；右键「查看源码」在文件 Tab 打开源码；地址栏导航、前进/后退、打开本地文件可用；无 scheme 输入补全正确（`www.google.com` → `https://www.google.com`，`localhost:5173` → `http://localhost:5173`，相对路径仍 `file://`）；远端页面无法跳转 `file://`；新开空白 Tab 显示欢迎页（地址栏空、标签条标题「新标签页」、原生视图隐藏），地址栏/欢迎页入口导航后欢迎页消失、视图显示（2026-09-18 增）
