...
| 2 | 快捷键体系 | Ctrl+T 新建 Tab、Ctrl+W 关闭激活 Tab（chat 沿用流式确认+归档语义）、Ctrl+Tab / Ctrl+Shift+Tab / Ctrl+PgUp / Ctrl+PgDn 切换（macOS 改惯例键 ⌘⌥←/→ 与 ⌘⇧[/]，Ctrl+Tab 系不绑定）。**Ctrl+Shift+T 恢复刚关闭的 Tab**：关闭栈记录已关 Tab，chat 恢复=重开（取消归档）、file/diff/browser 按标识重开、**终端恢复=在原 cwd 新建一个终端**（原 pty 已随关 Tab 销毁）；会话已被删除则跳过该项。**Alt+↑/↓ 在左栏项目/工作区间切换（mac ⌘⌥↑/↓；2026-09-04 修订替换 Ctrl+Alt+↑/↓——被 GNOME/KDE 合成器抢作工作区切换，应用收不到）**：按左栏显示顺序（项目行 → 其下工作区行，自上而下）移动当前作用域，等价于点击对应行（先切换后加载），到边界循环。不做 Ctrl+数字跳转（与系统/输入法快捷键易冲突） |
