...
| i18n | `discover*` 8 键 + `addProfileManualTitle` + `addProfileConnecting`（挂起 loading 文案，zh/en）+ `modeAttachDesc`/`modeManagedDesc` + `modeAttach`/`modeManaged` 改短标签（zh/en） |
| app.css | `.discover-candidate`（同 `.scan-candidate` 骨架 + main/徽标行内布局）、`.discover-actions`（space-between）、`.profile-mode-seg`/`.profile-mode-desc`、`.pending-connect`（修订 2 loading 行，spinner 复用 typing-spin）、欢迎屏 `.welcome-connecting*` |
