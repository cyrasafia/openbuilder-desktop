...
- 键盘排序、触屏长按排序
- ~~Tab 条拖拽迁移到本方案~~（2026-08-29 追记：**同日已反转该决策**——Tab 条拖拽与文件拖动引用均迁移到本模式。原依据"Tab 条横向单行、无死区问题，指示线模式够用且已验证"修订为：指示线模式虽无死区丢落位问题，但"指示线位置 ≠ 最终位置"仍需用户心智换算，与左栏预览式体验割裂。迁移落地见 [design-tab-drag-rename.md](./design-tab-drag-rename.md) §1 修订（几何判定水平化：X 轴边缘带 + 条尾空白 = 末位）、[design-file-reference.md](./design-file-reference.md) §3.3 修订（占位 chip 预览；提交仍挂 drop——复制语义松手在外 = 取消，与重排序"任何位置松手都提交"的取舍相反，dragend 在该场景只作占位清理兜底））
